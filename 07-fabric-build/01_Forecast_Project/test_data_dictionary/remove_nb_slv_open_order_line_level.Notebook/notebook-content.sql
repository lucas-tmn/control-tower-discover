...
/* SILVER LAYER: OPEN ORDERS - FACT TABLE
   Target: dbo.slv_open_order_line_level
   Source bronze Tables:
     - brz_wholesale_codis_afi__codatan               (order_detail codatan  → T1)
     - brz_wholesale_codis_afi__extorit  (order_item_detail_extended extorit  → T2)
     - brz_wholesale_codis_afi__comast                (order_header comast   → T3)
     - brz_wholesale_codis_afi__extord       (order_header_extended extord   → T4)
     - ref_item_master                      (dimitemmaster → IM)
     - ref_order_type                       (order_type aaordtyp → OT1-4)
   Legacy Reference: [SCP_CORE_WRK].[v_FactOpenOrders]
*/

CREATE OR REPLACE TABLE dbo.slv_open_order_line_level AS
SELECT
    /* ── Keys & Identifiers ── */
    T1.id_order,
...
FROM dbo.brz_wholesale_codis_afi__codatan                       AS T1 -- order detail

LEFT JOIN dbo.brz_wholesale_codis_afi__extorit    AS T2 -- order item detail extended
    ON  T1.id_order          = T2.id_order
    AND T1.num_item_sequence  = T2.num_item_sequence

INNER JOIN dbo.brz_wholesale_codis_afi__comast                 AS T3 -- order header
    ON  T1.id_order          = T3.id_order

INNER JOIN dbo.brz_wholesale_codis_afi__extord        AS T4 -- order header extended
    ON  T1.id_order          = T4.id_order

INNER JOIN dbo.ref_item_master                       AS IM
    ON  IM.id_item_sku       = T1.id_item_sku

LEFT JOIN dbo.ref_order_type                         AS OT1 -- order type
    ON  OT1.code_order_type  = T4.code_order_type_1

LEFT JOIN dbo.ref_order_type                         AS OT2 -- order type
    ON  OT2.code_order_type  = T4.code_order_type_2

LEFT JOIN dbo.ref_order_type                         AS OT3 -- order type
    ON  OT3.code_order_type  = T4.code_order_type_3

LEFT JOIN dbo.ref_order_type                         AS OT4 -- order type
...
SELECT * FROM SupplyChain_Lakehouse.dbo.slv_open_order_line_level LIMIT 100
